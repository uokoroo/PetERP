...
angular.module("cgpaCalculator").factory("cgpaFactory", function ($http) {
  ...
  function getStanding(data, gradeType) {
    ...
    data[gradeType + "ProgressBarWidth"] = (data[gradeType] / 4.0) * 100;
    return data;
  }
  ...
  function calculateGPAExpected(data, result) {
    ...
    var expectedTotalGradePoint =
      data["cgpaExpected"] *
      (data["currentCredits"] + data["creditsCompleted"]);

    result["gpaExpected"] = (
      (expectedTotalGradePoint - totalGradePoint) /
      data["currentCredits"]
    )
      .toFixed(4)
      .slice(0, -2);
  ...
  function calculateCGPA(data, result) {
    ...
    var totalGradePoint =
      data["creditsCompleted"] * data["cgpa"] + result["gradePoint"];
